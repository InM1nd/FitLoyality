  ...
  return (
    <div className="rounded-md border border-input bg-surface-3 px-3 py-2 shadow-[var(--shadow-elevated)]">
      ...
      <p className="num text-sm font-semibold text-foreground">
        {t("churnTooltip", { n: String(payload[0].value) })}
      </p>
    </div>
  );
}
  ...
  return (
    <Card className="flex h-full flex-col">
      <CardHeader className="flex-row items-center justify-between border-b border-border">
        <CardTitle>{t("churnTitle")}</CardTitle>
        <span className="text-xs text-faint">{t("churnSub")}</span>
      </CardHeader>
        ...
        <div className="h-[260px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={CHURN_SERIES} margin={{ top: 8, right: 8, bottom: 0, left: -20 }}>
              ...
              <YAxis tickLine={false} axisLine={false} tick={{ fill: "var(--text-3)", fontSize: 11 }} width={40} />
              <Tooltip content={<ChurnTooltip />} cursor={{ fill: "var(--surface-2)" }} />
              <Bar dataKey="churn" radius={[5, 5, 0, 0]} maxBarSize={36}>
                {CHURN_SERIES.map((d) => (
                  <Cell key={d.month} fill={d.churn === max ? "#ef4444" : "rgba(239,68,68,0.7)"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
